...
def day21_1():
    ...
    for line in lines:
        ...
        for op in ["*", "+", "-", "/"]:
            if op in second:
                ...
                for o in operands: edges.append((node, o,{"value": None}))
                break
        else:
    ...
    for i in range(1000):
        ...
        if i==0:
            ...
            assert len(zero_crossings) == 1, "Error: Not exactly one zero crossing detected. "\
        ...
        if err < 1e-7:
            break
        start, stop = vals[diffs == diffs[np.where(diffs>0)[0]].min()], vals[diffs == diffs[np.where(diffs<0)[0]].max()]
