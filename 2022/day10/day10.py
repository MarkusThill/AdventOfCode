...
def day10_1():
    file1 = open("2022/day10/input10_1.txt", "r")
    lines = file1.readlines()
    lines = [l.strip() for l in lines]
    lines = [l.split(" ") for l in lines]
    lines = [
        (1 if l[0] == "noop" else 2, int(l[1]) if len(l) > 1 else 0) for l in lines
    ]
    ...
    for c in range(1, 242):
        ...
        pixel_pos = (c - 1) % 40
        symbol = (
            "#" if pixel_pos >= sprite_pos - 1 and pixel_pos <= sprite_pos + 1 else "."
        )
        row.append(symbol)
