...
def tree2(G, node, valve_state, time_left, elephants_left, transposition):
    if time_left == 0:
        if elephants_left > 0:
            ...
            return tree2(G=G, node="AA", valve_state=valve_state, time_left=26, elephants_left=elephants_left - 1,
    ...
    if G.nodes[node]["flow"] > 0:
        ...
        if (valve_state & mask) == 0:  # If valve is not open already...
            ...
            result_new = (time_left - 1) * flow + tree2(G=G, node=node, valve_state=valve_state | mask,
                                                        time_left=time_left - 1, elephants_left=elephants_left,
                                                        transposition=transposition)
            result = max(result, result_new)

    # Move to another valve
    for s in G.successors(node):
        r = tree2(G=G, node=s, valve_state=valve_state, time_left=time_left - 1, elephants_left=elephants_left,
                  transposition=transposition)
        result = max(result, r)
...
def day16_1_2():
    ...
    start = time.time()
    ans = tree2(G=G, node="AA", valve_state=0, time_left=30, elephants_left=0, transposition=dict())
    end = time.time()
    print("Solution day 16.1:", ans, "! Time:", round(end - start, 2), "seconds")

    start = time.time()
    ans = tree2(G=G, node="AA", valve_state=0, time_left=26, elephants_left=1, transposition=dict())
    end = time.time()
